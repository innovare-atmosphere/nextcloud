...
resource "digitalocean_droplet" "www-nextcloud" {
  ...
  provisioner "file" {
    ...
    destination = "/root/nextcloud/db.env"
  }

  provisioner "file" {
    source      = "docker-compose.yml.tpl"
    destination = "/root/nextcloud/docker-compose.yml"
  }

  provisioner "file" {
    source      = "web/Dockerfile"
    destination = "/root/nextcloud/web/Dockerfile"
  }

  provisioner "file" {
    source      = "web/nginx.conf"
    destination = "/root/nextcloud/web/nginx.conf"
  }
